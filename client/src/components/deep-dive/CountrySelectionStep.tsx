...
export function CountrySelectionStep({
  ...
  const countriesByRegion = useMemo(() => {
    ...
    Object.entries(REGIONS).forEach(([regionId, region]) => {
      grouped[regionId] = countries.filter(c => 
        region.countries.includes(c.iso_code)
  ...
  const isRegionFullySelected = useCallback((regionId: string) => {
  ...
  const getRegionSelectedCount = useCallback((regionId: string) => {
    const regionCountries = countriesByRegion[regionId] || [];
    return regionCountries.filter(c => selectedCountries.includes(c.iso_code)).length;
  }, [countriesByRegion, selectedCountries]);

  // Handle region selection (select all countries in region)
  const handleSelectRegion = useCallback((regionId: string) => {
    ...
    if (isRegionFullySelected(regionId)) {
      // Deselect all countries in region
      ...
      <div className="h-full flex items-center justify-center relative overflow-hidden">
        ...
        >
          <div className="relative w-32 h-32 mx-auto mb-8">
            ...
            >
              <div className="absolute w-full h-full border-2 border-purple-500/20 rounded-full" />
            </motion.div>
            <motion.div
            ...
            >
              <div className="absolute w-full h-full border-2 border-cyan-500/30 rounded-full" />
            </motion.div>
                  ...
                  {searchResults.map((country, index) => (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        selectedCountries.includes(country.iso_code)
                          ? "bg-purple-500/20"
...
function GCCFeaturedCard({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 bg-gradient-to-r from-amber-500/5 via-amber-400/10 to-amber-500/5"
        animate={{
          x: ["-100%", "100%"],
        }}
        transition={{
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              className="p-2 rounded-lg bg-slate-800/50 border border-slate-700/40 text-slate-400 hover:text-white transition-colors"
            >
              <motion.div
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-1.5 bg-slate-800/60 rounded-full overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${(selectedCount / countries.length) * 100}%` }}
...
function RegionCard({
  ...
  const styles = colorStyles[region.color as keyof typeof colorStyles] || colorStyles.blue;
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className={cn(
              "w-10 h-10 rounded-lg bg-gradient-to-br border flex items-center justify-center",
              styles.iconBg
            )}>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 h-1 bg-slate-800/60 rounded-full overflow-hidden">
            <motion.div
              className={cn("h-full bg-gradient-to-r rounded-full", styles.progress)}
              initial={{ width: 0 }}
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isSelected
          ? `bg-${accentColor}-500/20 border-${accentColor}-500/40`
    ...
    >
      {country.flag_url ? (
        <img
          ...
          alt=""
          className={cn("object-cover rounded shadow-sm", compact ? "w-5 h-3" : "w-6 h-4")}
        />
      ) : (
        <span className={cn("text-slate-500", compact ? "text-[9px]" : "text-[10px]")}>
          {country.iso_code}
        </span>
      )}
      <span className={cn(

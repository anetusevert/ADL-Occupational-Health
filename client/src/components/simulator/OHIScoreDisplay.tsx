 ...
 * 
 * Branded score gauge with animated ring and maturity stage
 */
...
export function OHIScoreDisplay({
  score,
  previousScore,
  size = 'md',
  showDelta = true,
  ...
  // Calculate ring progress (1.0-4.0 mapped to 0-100%)
  const progressPercent = ((score - 1.0) / 3.0) * 100;
  
  // Size configurations
  const sizeConfig = {
    sm: { ring: 80, stroke: 6, fontSize: 'text-xl', labelSize: 'text-[10px]' },
    md: { ring: 120, stroke: 8, fontSize: 'text-3xl', labelSize: 'text-xs' },
    lg: { ring: 180, stroke: 10, fontSize: 'text-5xl', labelSize: 'text-sm' },
  };
    ...
    <div className={cn('flex flex-col items-center', className)}>
      ...
      <div className="relative" style={{ width: config.ring, height: config.ring }}>
        ...
        >
          <motion.circle
            ...
            strokeWidth={config.stroke}
            strokeLinecap="round"
            className={stageColors[stage.color]}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {/* ADL Logo Mark */}
          <div className={cn(
            'absolute -top-1 w-full flex justify-center',
          ...
          {/* Label */}
          <span className={cn('text-white/40 uppercase tracking-wider', config.labelSize)}>
            OHI Score
          </span>
      ...
      {showStage && (
        <motion.div
          ...
          className={cn(
            'mt-3 px-3 py-1 rounded-full border text-xs font-medium',
...
/**
 * Compact inline score display for headers/cards
 */
export function OHIScoreInline({
  score,
  previousScore,
  className,
}: {
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      <span className={cn('text-lg font-bold', stageColors[stage.color])}>
        {score.toFixed(2)}
      </span>
      {delta !== 0 && (
        <span className={cn(

...
export function calculatePolicyImpact(
  ...
  for (const policy of policies) {
    ...
    // Calculate impact based on current level
    // Diminishing returns: each level adds slightly less than the previous
    const diminishingFactor = 1 - (policy.currentLevel - 1) * 0.1; // 10% less per level
...
export function generateBaseRankings(playerCountry: CountryData): CountryRanking[] {
  // Sample countries with realistic OHI scores
  const baseCountries = [
    ...
    { iso: 'FIN', name: 'Finland', score: 3.5 },
    { iso: 'AUT', name: 'Austria', score: 3.4 },
    { iso: 'BEL', name: 'Belgium', score: 3.3 },
    { iso: 'IRL', name: 'Ireland', score: 3.2 },
    { iso: 'ESP', name: 'Spain', score: 2.9 },
    { iso: 'ITA', name: 'Italy', score: 2.8 },
    { iso: 'PRT', name: 'Portugal', score: 2.7 },
    { iso: 'POL', name: 'Poland', score: 2.5 },
    { iso: 'CZE', name: 'Czech Republic', score: 2.6 },
    { iso: 'GRC', name: 'Greece', score: 2.4 },
    { iso: 'ARE', name: 'UAE', score: 2.8 },
    { iso: 'SAU', name: 'Saudi Arabia', score: 2.3 },
    { iso: 'CHN', name: 'China', score: 2.4 },
    { iso: 'IND', name: 'India', score: 2.0 },
    { iso: 'BRA', name: 'Brazil', score: 2.2 },
    { iso: 'MEX', name: 'Mexico', score: 2.1 },
    { iso: 'RUS', name: 'Russia', score: 2.3 },
    { iso: 'TUR', name: 'Turkey', score: 2.4 },
    { iso: 'ZAF', name: 'South Africa', score: 2.1 },
    { iso: 'ARG', name: 'Argentina', score: 2.2 },
    { iso: 'THA', name: 'Thailand', score: 2.3 },
    { iso: 'MYS', name: 'Malaysia', score: 2.5 },
    { iso: 'IDN', name: 'Indonesia', score: 1.9 },
    { iso: 'VNM', name: 'Vietnam', score: 1.8 },
    { iso: 'PHL', name: 'Philippines', score: 1.9 },
    { iso: 'EGY', name: 'Egypt', score: 1.7 },
    { iso: 'NGA', name: 'Nigeria', score: 1.5 },
    { iso: 'PAK', name: 'Pakistan', score: 1.6 },
    { iso: 'BGD', name: 'Bangladesh', score: 1.5 },
    { iso: 'KEN', name: 'Kenya', score: 1.6 },
    { iso: 'ETH', name: 'Ethiopia', score: 1.3 },
  ];
  
  // Remove player country if it exists in the list
 ...
 * Higher GDP = more budget points, with logarithmic scaling
 */
export function calculateBudgetPoints(country: CountryData): number {
  // Base formula: logarithmic scaling of GDP
  // Small countries (GDP < 50B): 100-300 points
  // Medium countries (GDP 50B-500B): 300-600 points
  // Large countries (GDP 500B-5T): 600-900 points
  // Major economies (GDP > 5T): 900-1200 points
  
  const gdpBillions = country.gdp;
  const healthPct = country.healthExpenditure / 100;
  
  // OH budget is typically 0.1-0.5% of health expenditure
  const ohBudgetFactor = 0.003; // 0.3% of health expenditure
  
  // Calculate base points
  const basePoints = Math.log10(gdpBillions * healthPct * ohBudgetFactor * 1000 + 1) * 150;
  
  // Add bonus for formal sector coverage (more formal = easier to implement)
...
export function simulateCycle(
  ...
  // 5. Apply changes to pillar scores
  ...
  const newRank = playerRanking?.currentRank || state.rankings.find(r => r.isPlayer)?.currentRank || 50;
...
/**
 * Calculate natural score changes based on investment levels
 * Under-investment leads to decay, over-investment leads to growth
 */
function calculateNaturalChange(pillars: PillarScores, budget: BudgetState): PillarDelta {
  ...
  // For each pillar, calculate natural change based on investment ratio
  ...
  for (const pillarId of pillarsArray) {
    ...
    if (ratio < 0.5) {
      // Severe under-investment: decay
      delta[pillarId] = -2;
    } else if (ratio < 0.8) {
      // Moderate under-investment: slight decay
      delta[pillarId] = -1;
    } else if (ratio > 1.2) {
      // Over-investment: bonus growth
      delta[pillarId] = 0.5;
    }
    // 0.8-1.2: stable, no natural change
  }
...
const ACHIEVEMENT_DEFINITIONS: Omit<Achievement, 'unlockedYear' | 'isUnlocked'>[] = [
  ...
  { id: 'crisis_manager', name: 'Crisis Manager', description: 'Successfully handle 5 critical events', icon: 'AlertTriangle' },
 ...
 * Base probability with modifiers for game state
 */
export function shouldTriggerEvent(state: GameState): boolean {
  // Base 40% chance per cycle
  let probability = 0.4;
  
  // Increase if no recent events
  ...
  // Increase for extreme scores (very low or very high)

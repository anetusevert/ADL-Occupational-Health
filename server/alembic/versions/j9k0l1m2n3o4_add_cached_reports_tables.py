...
def upgrade() -> None:
    # Create cached_pillar_reports table (idempotent)
    if not table_exists('cached_pillar_reports'):
        op.create_table(
    ...
    # Create cached_summary_reports table (idempotent)
    if not table_exists('cached_summary_reports'):
        op.create_table(
